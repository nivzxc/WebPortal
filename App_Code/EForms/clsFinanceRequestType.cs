...
namespace STIeForms
{
 public class clsFinanceRequestType : IDisposable
 {
  ...
  public static DataTable GetDSL()
  {
   ...
   using (SqlConnection cn = new SqlConnection(clsSystemConfigurations.ConnectionStringMRCF))
   {
    ...
    cmd.CommandText = "SELECT rqstcode as pValue, rqstname as pText FROM Finance.RequestType ORDER BY pText";
    cn.Open();
  ...
  public static string GetRequestTypeName(string pRequestTypeCode)
  {
   ...
   {
    SqlCommand cmd = cn.CreateCommand();
    cmd.CommandText = "SELECT rqstname FROM Finance.RequestType WHERE rqstcode =@rqstcode";
