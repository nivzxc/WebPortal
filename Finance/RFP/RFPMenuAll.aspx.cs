...
public partial class Finance_RFP_RFPMenuAll : System.Web.UI.Page
{
 ...
 protected void LoadPaging()
 {
  ...
  using (SqlConnection cn = new SqlConnection(ConfigurationManager.ConnectionStrings["Speedo"].ToString()))
  {
   ...
   cmd.CommandText = "SELECT COUNT(ctrlnmbr) AS tcount FROM Finance.RFPRequest WHERE createby='" + Request.Cookies["Speedo"]["UserName"] + "' ";
  ...
  while (intTRowsTemp > 0)
  {
   if (Convert.ToInt32(Request.QueryString["page"]) == intPage)
    ...
    Response.Write("&nbsp;&nbsp;<a href='RFPMenuAll.aspx?page=" + intPage + "'>" + intPage + "</a>");
   intPage++;
   intTRowsTemp -= intPageSize;
  }
 }

 protected void LoadPagingFinance()
 {
  int intPageSize = Convert.ToInt32(ConfigurationManager.AppSettings["pagesize"]);
  int intTRows = 0;
  int intTRowsTemp = 0;
  ...
  {
   SqlCommand cmd = cn.CreateCommand();
   cmd.CommandText = "SELECT COUNT(ctrlnmbr) AS tcount FROM Finance.RFPRequest";
 ...
 public void LoadPagingApprover()
 { }
 ...
 protected void LoadMenuRFP()
 {
  if (!IsPostBack)
  {
   ...
   DataTable tblRFP = clsRFPRequest.GetDSGMainFormPerUser(" ctrlnmbr, rqstcode, rqstfor, projttle, dateneed, payename,endrsby1,endrstt1,endrstt2,endrsby2,authrzby,authstat,statcode, createby, createon", Request.Cookies["Speedo"]["UserName"], strWhere, intStart, intEnd);
 ...
 protected void LoadMenuRFPFinance()
 {
  if (!IsPostBack)
  {
   if (clsSystemModule.HasAccess("023", Request.Cookies["Speedo"]["UserName"].ToString().Trim()))
   {
    ...
    DataTable tblRFP = clsRFPRequest.GetDSGMainForm(" ctrlnmbr, rqstcode, rqstfor, projttle, dateneed, payename,endrsby1,endrstt1,endrstt2,endrsby2,authrzby,authstat,statcode, createby, createon", strWhere, intStart, intEnd);
    foreach (DataRow drw in tblRFP.Rows)
    {
     ///////////////////////////////////////////No Endorser////////////////////////////////////////////////////
     if (clsFinanceApprover.IsHaveNoEndorser("ctrlnmbr", drw["ControlNumber"].ToString(), "RFPRequest"))
